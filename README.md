...
4. Parameters.xml (and included files in it)
	...
	* mainParams
	...
	doWhat = "NOTHING", "SAVE_RAW", "IMAGE_PROC", or "SAVE_ROS_BAG" (if ros exists)
	
	rosBagCompression = "ture", "false"
		
	Below is the frequency and log size results of the program running on a laptop with an Intel Core-i7-7500 cpu and an NVIDIA GeForce 940MX gpu.
	
	253 tile of size 256x256 were loaded and rendered in this experience. Motion blur was disabled.
		
	A USB3.0 external storage was used for storing logs. Using a USB2.0 instead of a USB3.0 flash lowers fps by 50%
	
	Notice that the program will firstly use the flash drive for storing log files, to prevent extreme harddisk usage. 

	
	| doWhat | fps | log size(100x(640x480-mono) images) |
	| ----------- | ----------- | ----------- |
	| NOTHING | 80 | 0MB |
	| SAVE_RAW(bmp) | 38 | 88MB |
	| SAVE_RAW(png) | 19 | ~9MB |
	| SAVE_ROS_BAG(compressed) | 25 | ~9MB |
	| SAVE_ROS_BAG(uncompressed) | 50 | ~31MB |
	
	The log would be very bulky, in order of 200MB/min, in the non-compressed rosbag case, for a 10fps grayscale 640x480 camera. The color image would even need 4x bandwidth.
